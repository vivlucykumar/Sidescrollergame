...
export function useScrollDevice() {
    ...
    useEffect(() => {
        ...
        let lastDeltaY = 0;
        const detectScrollDevice = (event) => {
            const now = performance.now();
            const timeDiff = now - lastEventTimestamp;
            const isTrackpad = (Math.abs(event.deltaY) < 50 || // Small delta values indicate trackpad
                (Math.abs(event.deltaX) > 0 && Math.abs(event.deltaY) < 50) // Trackpads scroll in both directions
            );
            const isMouseWheel = (Math.abs(event.deltaY) >= 50 || // Large jumps indicate mouse wheel
