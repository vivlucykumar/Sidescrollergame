...
describe('useWebSocket', () => {
    ...
    it('should update status to CLOSED when connection is closed and reconnection attempts are exhausted', () => {
        const { result } = renderHook(() => useWebSocket(url, { maxReconnectAttempts: 0 })); // Disable reconnection
        act(() => {
    ...
    it('should attempt to reconnect when connection is closed', () => {
        ...
        const setTimeoutSpy = jest.spyOn(window, 'setTimeout'); // Spy on setTimeout
        const { result } = renderHook(() => useWebSocket(url, { maxReconnectAttempts: 3 }));
        act(() => {
            mockWebSocket.onclose({});
        });
    ...
    it('should stop reconnecting after maxReconnectAttempts', () => {
        jest.useFakeTimers(); // Mock timers
        const setTimeoutSpy = jest.spyOn(window, 'setTimeout'); // Spy on setTimeout
        const { result } = renderHook(() => useWebSocket(url, { maxReconnectAttempts: 2 }));
